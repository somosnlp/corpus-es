...
def load_dataset(dataset_path: str) -> None:
    """
    Load a dataset from the provided path llowing these steps:
    1. Download JSON and convert it into a CSV.
    ...
    # make the translated folder if it doesn't exist
...
def generate_batches(origin: str, destination: str, columns_for_translation: list):
    ...
    for column in columns_for_translation:
        new_df[column] = new_df.groupby("original_id")[column].transform(
            concat_translations
        )
...
if __name__ == "__main__":
    # download_and_open_dataset('scidocs') this is only if u haven't downloaded one of the retrieval datasets from the beir benchmark
    ...
    parser = argparse.ArgumentParser(
        description="Load a dataset from Hugging Face and add unique IDs if not present."
    )
    parser.add_argument(
        "-p",
        "--path",
        type=str,
        required=True,
        help="path to desired json you want to translate",
    )
    parser.add_argument(
        "-c", "--columns", type=list, required=True, help="columns to translate"
    )
    ...
    generate_batches(
        origin=original_csv_path,
        destination=destinaton_csv_path,
        columns_for_translation=args.columns,
    )
